...
public class Memoria{
	 ...
	 * @param addr
	 *          O endereço no qual o Bits8 será escrito (entre MIN_ADDRESS e
	 *          MAX_ADDRESS).
	 * @throws MVNException
	...
	public void write(Bits8 p, int addr) throws MVNException{
        ...
        /**
	 * Escreve uma palavra (dois bytes) em um endereço da memória. <br>
	 * <b>Pré-condição</b>: a Word a ser escrita não pode ser null. <br>
	 * <b>Pós-condição</b>: as posições de memória <i>addr</i> e <i>addr+1</i>
         * contêm o valor <i>w</i> (hiWord e LoWord, respectivamente).
	 * 
	 * @param p
	 *          A palavra a ser escrita.
	 * @param addr
	 *          O endereço a partir do qual a Word será escrita (entre MIN_ADDRESS e
	 *          MAX_ADDRESS).
	 * @throws MVNException
	 *           Caso o endereço de memória especificado seja inválido.
	 */
	public void writeWord(Word w, int addr) throws MVNException{
		// Verificar se o endereço é válido.
		if(!isValidAddress(addr) || !isValidAddress(addr+1)){
			throw new MVNException(ERR_INVALID_ADDRESS, addr);
		}
                
		store[addr - minAddress] = w.getHiWord();
                store[addr - minAddress + 1] = w.getLoWord();
		return;
	}
        
	
	/**
	 * Retorna um Bits8 armazenado em um endereço da memória.<br/>
	 * <br/>
	...
	public Bits8 read(int addr) throws MVNException{
        ...
        /**
	 * Retorna uma Word (2 bytes) armazenada em um endereço da memória.<br/>
	 * <br/>
	 * <b>Pré-condição</b>: <i>addr</i> deve ser um endereço válido.<br/>
	 * <b>Pós-condição</b>: O endereço é lido.
	 * 
	 * @param addr
	 *          O endereço de memória da Word a ser retornada.
	 * @return A Word lida da memória.
	 * @throws MVNException
	 *           Caso o endereço especificado seja inválido.
	 */
	public Word readWord(int addr) throws MVNException{
		if(!isValidAddress(addr) || !isValidAddress(addr+1)){
			throw new MVNException(ERR_INVALID_ADDRESS, addr);
		}
		return new Word(store[addr - minAddress + 1], store[addr - minAddress]);
	}
