...
bool consome_token(APE *ape, Token *token) {
    ...
    if (transicao != NULL) {
        /* Realiza a transição */
        semantico(automato, token, transicao);
    ...
    if (chamada != NULL) {
        ...
        semantico_submaquina(automato, token, chamada);
        automato->estado = chamada->estadoResultado;

        /* Empilha automato e consome token novamente */
        if (strcmp(chamada->submaquina, "STACK_IDENTIFIER") != 0
            &&
            strcmp(chamada->submaquina, "NUMBER") != 0) {
    ...
    if (is_final(automato)) {
        ...
        semantico_desempilha(automato, token, ape);
